...
export default function Main({ navigation, route }) {
    ...
    try {
      ...
      if (data !== null) {
        ...
        for (const consignorCode in parsedData) {
          if (
            parsedData.hasOwnProperty(consignorCode) &&
            parsedData[consignorCode].acceptedItems11
          ) {
            const acceptedItemsLength =
              parsedData[consignorCode].acceptedItems11.length;
            totalLength += acceptedItemsLength;
          }
        }
        setAcceptedItemData(totalLength);
        console.log(
          "Main.js/loadAcceptedItemData12 ",
          "Total length of acceptedItems11:",
          totalLength
        );
      }
    } catch (error) {
  ...
  const fetchCloseHandoverStatus = async () => {
    const decodedToken = token ? jwtDecode(token) : null;
    const currentEpochTime = Math.floor(Date.now() / 1000);
    if (id && decodedToken?.exp >= currentEpochTime) {
      try {
        // console.log('Main.js/ ',id);
        await axios
          .get(`${backendUrl}/SellerMainScreen/handoverStatus?feUserID=${id}`, {
            headers: getAuthorizedHeaders(token),
          })
          .then((response) => {
          ...
          .catch((error) => {
            console.log(
              "Main.js/fetchCloseHandoverStatus ",
              "Error Msg:",
              error
            );
          });

          ...
          for (let i = 0; i < results.rows.length; i++) {
            ...
            try {
              const response = axios
                .post(
                  backendUrl + "SellerMainScreen/partialDispatch",
                  {
              ...
              if (i === results.rows.length - 1) {
                db.transaction((tx) => {
                  tx.executeSql(
                    'UPDATE closeBag1 SET status="dispatched"  WHERE status="Scanned" AND stopId=?  ',
                    [route.params.stopId],
                    (tx1, results12) => {
                      if (results12.rowsAffected > 0) {
                        ToastAndroid.show(
                          "Bags Dispatch Successfully",
                          ToastAndroid.SHORT
                        );
              ...
              txn.executeSql(
                ...
                (sqlTxn, res) => {
                  ...
                  if (res.rows.length > 0) {
                    ...
                    db.transaction((tx) => {
                      tx.executeSql(
                        'UPDATE closeBag1 SET status="Scanned" WHERE bagSeal=? And stopId=?',
                        [bagSeal, route.params.stopId],
                        (tx1, results) => {
                          // let temp = [];
                          if (results.rowsAffected > 0) {
          ...
          <ScrollView showsVerticalScrollIndicator={true}>
            ...
            >
              ...
              {Object.keys(sealIDData11).map((sealID, index) => (
                ...
                >
                  <Text style={{ fontSize: 16, fontWeight: "500" }}>
                    {sealIDData11[sealID].bagID}
                  </Text>
                  <Text style={{ fontSize: 16, fontWeight: "500" }}>
                    {sealIDData11[sealID].acceptedItemsCount}
                  </Text>
                </View>
              ))}

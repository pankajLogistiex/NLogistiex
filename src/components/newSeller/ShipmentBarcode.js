...
var dingAccept = new Sound(dingAccept11, error => {
  ...
  const updateDetails2 = (expectedPackagingId, stopId) => {
    ...
    db.transaction(tx => {
      tx.executeSql(
        'UPDATE SellerMainScreenDetails SET status="accepted", packagingId=?, expectedPackagingId=?, eventTime=?, latitude=?, longitude=?, stopId=?, rejectionReasonL1="", postRDStatus="false", syncStatus="" WHERE (awbNo=? OR clientRefId=? OR clientShipmentReferenceNumber=?) AND FMtripId=?',
        [
          packagingID,
          expectedPackagingId,
          new Date().valueOf(),
